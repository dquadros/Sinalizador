...
// Ex: "SinalCmdLnx RG" acende os LEDS vermelho e verde e apaga os demais
//
    ...
    for(i = 0; i < 32; i++) {
        ...
        if (fd >= 0){
            ...
            if ((device_info.vendor == MEU_VID) &&
                ...
                printf ("Dispositivo achado em %s", path);
                return fd;
            }
            close (fd);
        }
...
int SetFeature (int fd, int leds) {
	...
	ref_multi_u.values[0] = 1;  // identificação do dispositivo
	ref_multi_u.values[1] = 0;		// 
	ref_multi_u.values[2] = leds;	// programação dos leds
